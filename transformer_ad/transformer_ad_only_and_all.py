...
class Transformer_AD_only_and_all(Algorithm, PyTorchUtils):
...
class RelationNet(nn.Module, PyTorchUtils):
    def __init__(self, input_dim: int, hidden_dim: int = 10,
        ...
        if no_longterm and no_featerm:
            ...
            #submodules_for_decoders = [CoderBlock(sfa_dim, sfa_dim * int(dims[-1]//hidden_dim), dropout=dropout)]
            nhead=head if sfa_dim % head == 0 else 1
            submodules_for_decoders = [TransformerEncoderLayer(d_model=sfa_dim, nhead=nhead, dim_feedforward=sfa_dim * int(dims[-1]//hidden_dim), dropout=dropout)]
            for d in range(1, len(dims)):
                #submodules_for_decoders.append(CoderBlock(sfa_dim * int(dims[-d]//hidden_dim), sfa_dim * int(dims[-d-1]//hidden_dim), dropout=dropout))
                nhead=head if (sfa_dim * int(dims[-d]//hidden_dim)) % head == 0 else 1
                submodules_for_decoders.append(TransformerEncoderLayer(d_model=sfa_dim * int(dims[-d]//hidden_dim), nhead=nhead, dim_feedforward=sfa_dim * int(dims[-d-1]//hidden_dim), dropout=dropout))
            self.decoders = nn.ModuleList(submodules_for_decoders)
            self.final_decoder = nn.Linear(sfa_dim * int(dims[0]//hidden_dim), input_dim)
        else:
    ...
    def forward(self, x, sfa=None):
        ...
        if self.use_sfa:
            ...
            x_reconstruct = z.transpose(0,1)
            for d in self.decoders:
                x_reconstruct = d(x_reconstruct)
            x_reconstruct = self.final_decoder(x_reconstruct.transpose(0,1))
