...
class Transformer_VAE_AD(Algorithm, PyTorchUtils):
    ...
    def fit(self, data_list: Dataset, valid_data_list: Dataset, loss_func: str, log_step: int = 2, patience: int = 2):
        '''
        input:
        ...
        for epoch in trange(self.num_epochs):
            ...
            for (i, batch) in enumerate(tqdm(train_loader)):
                ...
                if self.use_sfa:
                    x_reconstruct, x_origin, z_mu, z_logvar = self.ed(self.to_var(x), self.to_var(sfa))
                else:
                    x_reconstruct, x_origin, z_mu, z_logvar = self.ed(self.to_var(x))
                ...
                if loss_func == 'rank':
                    ...
                    total_loss, _, _ = self.ed.loss_func_mse(x_reconstruct, x_origin, z_mu, z_logvar)
                elif loss_func == 'mae':
                    total_loss, _, _ = self.ed.loss_func_mae(x_reconstruct, x_origin, z_mu, z_logvar)
                elif loss_func == 'dtw':
                    total_loss = self.ed.loss_func_sdtw(x_reconstruct, x_origin, z_mu, z_logvar)
                elif loss_func == 'huber':
                    total_loss, _, _ = self.ed.loss_func_huber(x_reconstruct, x_origin, z_mu, z_logvar)
                elif loss_func == 'robust':
                    total_loss = self.ed.loss_func_robust(x_reconstruct, x_origin, z_mu, z_logvar)
    ...
    def predict(self, dataslices: Dataset):
        '''
        ...
        for (i, batch) in enumerate(tqdm(data_loader)):
            ...
            if self.use_sfa:
                ...
                if self.loss_func == 'mse':
                    x_reconstruct, output_logvar = torch.split(x_reconstruct, self.ed.input_dim, dim=1) 
                    loss = self.ed.mse(x_reconstruct, x_origin)
                    output = torch.div(loss, output_logvar.exp()) + output_logvar + np.log(np.pi*2)
                elif self.loss_func == 'mae':
                    x_reconstruct, output_logvar = torch.split(x_reconstruct, self.ed.input_dim, dim=1) 
                    loss = self.ed.mae(x_reconstruct, x_origin)
                    output = torch.div(loss, output_logvar.exp()) + output_logvar + np.log(np.pi*2)
                elif self.loss_func == 'huber':
                    x_reconstruct, output_logvar = torch.split(x_reconstruct, self.ed.input_dim, dim=1) 
                    loss = self.ed.huber(x_reconstruct, x_origin)
                    output = torch.div(loss, output_logvar.exp()) + output_logvar + np.log(np.pi*2)
            else:
                x_reconstruct, x_origin, z_mu, z_logvar = self.ed(self.to_var(x))
                if self.loss_func == 'mse':
    ...
    def reconstruct(self, dataslices: Dataset):
        ...
        for (i, batch) in enumerate(tqdm(data_loader)):
            ...
            if self.use_sfa:
                x_reconstruct, x_origin, _, _ = self.ed(self.to_var(x), self.to_var(sfa))
                output = x_reconstruct
            else:
                x_reconstruct, x_origin, _, _ = self.ed(self.to_var(x))
                output = x_reconstruct
...
class RelationNet(nn.Module, PyTorchUtils):
    def __init__(self, input_dim: int, hidden_dim: int = 10,
        ...
        submodules.append(nn.Linear(dims[-1], sfa_dim*2))
        ...
        if not no_featerm:
            ...
            fea_encoder_layers.append(nn.Linear(dims[-1], sfa_dim*2))
        ...
        if not no_longterm:
            ...
            longterm_encoder_layers.append(nn.Linear(dims[-1], sfa_dim*2))
        ...
        if no_longterm and no_featerm:
            ...
            self.final_decoder = nn.Linear(sfa_dim * int(dims[0]//hidden_dim), input_dim*2)
    ...
    def forward(self, x, sfa=None):
        ...
        # decoder
        if self.use_sfa:
            z_mu, z_logvar = torch.split(z, z.shape[2]//2, dim=2) 
            x_reconstruct = self.reparameterize(z_mu, z_logvar, self.training)
        ...
        return x_reconstruct, x_origin, z_mu, z_logvar
    ...
    def loss_func_mse(self, x_reconstruct, x_origin, mu, logvar):
        x_reconstruct, output_logvar = torch.split(x_reconstruct, self.input_dim, dim=1) 
        loss = self.mse(x_reconstruct, x_origin)
        recon_loss = 0.5 * torch.mean(torch.sum(torch.div(loss, output_logvar.exp()) + output_logvar + np.log(np.pi*2), (1,2)))
        kl_loss = -0.5 * torch.mean(torch.sum(1 + logvar - mu**2 - logvar.exp(),(1,2)))
        total_loss = recon_loss + kl_loss
        return total_loss, recon_loss, kl_loss
    
    def loss_func_mae(self, x_reconstruct, x_origin, mu, logvar):
        x_reconstruct, output_logvar = torch.split(x_reconstruct, self.input_dim, dim=1) 
    ...
    def loss_func_huber(self, x_reconstruct, x_origin, mu, logvar):
        x_reconstruct, output_logvar = torch.split(x_reconstruct, self.input_dim, dim=1) 
        loss = self.huber(x_reconstruct, x_origin)*2
        recon_loss = 0.5 * torch.mean(torch.sum(torch.div(loss, output_logvar.exp()) + output_logvar + np.log(np.pi*2), (1,2)))
        kl_loss = -0.5 * torch.mean(torch.sum(1 + logvar - mu**2 - logvar.exp(),(1,2)))
        total_loss = recon_loss + kl_loss
        return total_loss, recon_loss, kl_loss
